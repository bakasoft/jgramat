...
public class EvalContext {
    ...
    private boolean open_edit(Expression expression, Function<Location, Edit> editMaker) {
        ...
        add(editMaker.apply(loc0));
        if (expression.eval(this)) {
            var posF = source.getPosition();
            add(new EditCloseValue(source.locationOf(posF)));
            return true;
        }
        return false;
    }
    ...
    public void sendFragment(String value) {
        add(new EditSendFragment(source.getLocation(), value));
    }

    public void sendSegment(int pos0, int posF, ValueParser parser) {
        add(new EditSendSegment(source.locationOf(pos0), pos0, posF, parser));
    }

    public void sendValue(Object value) {
        add(new EditSendValue(source.getLocation(), value));
    }

    public void mark(int pos0, int posF, String name) {
        add(new EditMark(source.locationOf(pos0), pos0, posF, name));
    }

    public void set(Location location, String name) {
        add(new EditSet(location, name));
    }
