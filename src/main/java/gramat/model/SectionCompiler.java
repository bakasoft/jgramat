...
public class SectionCompiler extends DefaultComponent {
    ...
    public SectionCompiler(Component parent, GraphGrammar graphGrammar, SectionGrammar sectionGrammar) {
        super(parent);
    ...
    public Section compile(String name) {
        var graph = graphGrammar.findGraph(name);
    ...
    private void deep_copy(Graph graph, Section section, List<Runnable> promises) {
        ...
        for (var edge : graph.edges) {
            ...
            if (edge instanceof EdgeSymbol) {
                ...
                var refGraph = graphGrammar.findGraph(edgeReference.name);

                if (graphGrammar.isFlat(refGraph)) {
